...
var clearBoard = function () {
    ...
    var gameB = document.getElementById("gameboard");
    while (gameB.firstChild) {
        gameB.removeChild(gameB.firstChild);
    }
...
var openCell = function (row, col) {
    ...
    if (cell.getAttribute("marked") == "true") {
        ...
        var content = createBackFaceContent(row, col);
    ...
    for (var row = 0; row < GAME.rows; row++) {
        for (var col = 0; col < GAME.cols; col++) {
           ...
           if (cell.getAttribute("marked") == "true" ) {
            ...
            if ((cell.getAttribute("marked") == "true") && GAME.layout[row][col] != -1) {
                console.log(cell.getAttribute("marked"));
...
var updateHallOfFame = function (time, outcome) {
    ...
    console.log(toonid);
    if (outcome == false) {
        //document.getElementById("toonid").classList.remove("icon-emo-sleep");
        document.getElementById(toonid).className = "gametoon icon-emo-unhappy";
        document.getElementById(timeid).innerText = "you lost";
    }
    else {
        document.getElementById(toonid).className = "gametoon icon-emo-happy";
...
var updateBombsRemaining = function (value) {
    var bc = document.getElementById("bombs");
    var count = format(value, 2);
    // alert(count);
    bc.innerText = count;
...
var indicateNeighbors = function (row, col) {
    ...
    for (i = row - 1; i <= row + 1; i++) {
        for (j = col - 1; j <= col + 1; j++) {
            var cell = GAME.Cells[i][j];
            if (i != row || j != col && cell.classList.contains("closed"))
...
var handleMouseEvents = function (mouseButton, row, col, marked) {
    ...
    var tco = (GAME.rows * GAME.cols) - GAME.bombs;
    var tc = GAME.openCellCount;
    if (tco == tc) {
        gameWon();
    }
    //       call gameWon() method
...
var updateNeighborCounts = function (row, col) {
    ...
    if (col == 0)
        ...
        l = col - 1;
    for (i = k; i <= row + 1 && i < GAME.rows; i++) {
        for (j = l; j <= col + 1 && j < GAME.cols; j++) {

            if ((GAME.layout[i][j] != -1) && (i != row || j != col)) {
                console.log("updating neighbour" + i + "," + j);
                GAME.layout[i][j] += 1;
            }
        }
    }
...
var setBombs = function (row, col) {
    ...
    for (var count = 0; count < GAME.bombs; count++) {
        ...
        // console.log(i + "," + j);
        if ((GAME.layout[i][j] != -1) && (i != row && j != col && i != row - 1 && j != col - 1 && j != col + 1 && i != row + 1 && j != col + 1)) {
            GAME.layout[i][j] = -1;
...
var elapsedTimeInSeconds = function () {
    var x = new Date();
    //console.log( Math.floor((x - GAME.startTime) / 1000));
